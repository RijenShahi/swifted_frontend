...
const ResetPassword = (props) => {
    ...
    let [resetPassword, setResetPassword] = useState({
        "newPassword": "",
        "confirmPassword": "",
        "resetToken": props.match.params.tokenId

    })

    const resendLink = (e) => {
        if (localStorage.getItem("email")) {
            axios.post("http://localhost:90/swiftedAPI/userProfile/verifyEmail", { "email": localStorage.getItem("email") })
                .then((response) => {
    ...
    const resetThePassword = (e) => {
        e.preventDefault();
        axios.post("http://localhost:90/swiftedAPI/userProfile/reset/Password", resetPassword)
            .then((response) => {
